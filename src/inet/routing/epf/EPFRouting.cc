...
#include "inet/routing/epf/EPFRouting.h"
...
Define_Module(EPFRouting);

void EPFRouting::initialize(int stage)
{
    ...
    enquireSentSignal = registerSignal("enquireSent");
    enquireRecSignal = registerSignal("enquireRec");
    respSentSignal = registerSignal("respSent");
    respRecSignal = registerSignal("respRec");
    respOkSentSignal = registerSignal("respOkSent");
    respNOkSentSignal = registerSignal("respNOkSent");
    respOkRecSignal = registerSignal("respOkRec");
    respNOkRecSignal = registerSignal("respNOkRec");
    destEnqSignal = registerSignal("destEnq");
    destRespSignal = registerSignal("destResp");
    dataSentSignal = registerSignal("dataSent");
    dataRecSignal = registerSignal("dataRec");
    dataDestSignal = registerSignal("dataDest");
    dataFwdSignal = registerSignal("dataFwd");
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        EV << "host index: " << host->getIndex() << endl;
        ...
        enquire = createEnquireMessage();

        if (isOperational){
            scheduleAt(simTime() + 1, counterTimer);
            double delay = 3+host->getIndex()/10;
            //if(!strcmp(getSelfIPAddress(),"145.236.0.1"))
            scheduleAt(simTime()+delay,enquire);
        }
    }
}

void EPFRouting::handleMessage(cMessage *msg)
{

    EV << "Message arrived on epfrouting" << endl;
   ...
   if (!isOperational) {
    ...
    if (msg->isSelfMessage()) {
        if (dynamic_cast<WaitForRREP *>(msg))
            ...
            rreqCount = rerrCount = 0;
            scheduleAt(simTime() + 1, counterTimer);
            CONTACTINFOMSG *contactMsg = createCONTACTINFOMSG();
            sendEPFPacket(contactMsg,addressType->getBroadcastAddress(),1,0);
        }
...
//            enquire->setSeqNum(seqNum);
//            seqNum++;
            sendEnquireMessage(msg);
            enquire = createEnquireMessage();
            double delay = 3+host->getIndex()/10;
            scheduleAt(simTime()+delay,enquire);
        }else
        ...
        if(strncmp("ICMP",msg->getName(),strlen("ICMP"))){
            ...
            EPFControlPacket *ctrlPacket = check_and_cast<EPFControlPacket *>(udpPacket->decapsulate());
            ...
            switch (ctrlPacket->getPacketType()) {
                case RREQ:
                    ...
                    handleRREQ(check_and_cast<EPFRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                    break;

                case RREP:
                    rrepRec++;
                    emit(rrepRecSignal,rrepRec);
                    handleRREP(check_and_cast<EPFRREP *>(ctrlPacket), sourceAddr);
                    break;

                case RERR:
                    handleRERR(check_and_cast<EPFRERR *>(ctrlPacket), sourceAddr);
                    break;
                ...
                case Enquire:
                    enquireRec++;
                    emit(enquireRecSignal,enquireRec);
                    handleEnquireMessage(check_and_cast<EnquireMessage*>(ctrlPacket));
                    break;
                case Response:
                    respRec++;
                    emit(respRecSignal,respRec);
                    handleResponseMessage(check_and_cast<EnquireResponse*>(ctrlPacket));
                    break;
                case Data:
                    dataRec++;
                    emit(dataRecSignal,dataRec);
                    handleDataMessage(check_and_cast<DataMessage*>(ctrlPacket));
                    break;
                default:
                    throw cRuntimeError("EPF Control Packet arrived with undefined packet type: %d", ctrlPacket->getPacketType());
            }
            delete udpPacket;
        }
    }
...
/*void EPFRouting::initializeSocianNetworks(){
    int i;
...
INetfilter::IHook::Result EPFRouting::ensureRouteForDatagram(INetworkDatagram *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        EPFRouteData *routeData = route ? dynamic_cast<EPFRouteData *>(route->getProtocolData()) : nullptr;
...
EPFRouting::EPFRouting()
{
}

bool EPFRouting::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void EPFRouting::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
    ...
    EPFRREQ *rreq = createRREQ(target);
    addressToRreqRetries[target] = 0;
    sendRREQ(rreq, addressType->getBroadcastAddress(), timeToLive);
}

L3Address EPFRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void EPFRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void EPFRouting::sendRREQ(EPFRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendEPFPacket(rreq, destAddr, timeToLive, jitterPar->doubleValue());
    rreqCount++;
    rreqSent++;
    emit(rreqSentSignal,rreqSent);
}

void EPFRouting::sendRREP(EPFRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    EPFRouteData *destRouteData = check_and_cast<EPFRouteData *>(destRoute->getProtocolData());
    ...
    sendEPFPacket(rrep, nextHop, timeToLive, 0);
    rrepSent++;
    emit(rrepSentSignal,rrepSent);
}

void EPFRouting::sendEnquireMessage(cMessage* msg){
    int destIndex = intuniform(0,numHosts-1);
    while(destIndex == host->getIndex())
        destIndex = intuniform(0,numHosts-1);
    std::string addr = "host[";
    std::string index = std::to_string(destIndex);
    index.append("]");
    addr.append(index);
    const char* dest = addr.c_str();
    L3AddressResolver *res = new L3AddressResolver();
    L3Address destAddr;
    res->tryResolve(dest,destAddr);
    EnquireMessage* enqMsg = check_and_cast<EnquireMessage*>(msg);
    enqMsg->setDestAddr(destAddr);
    enqMsg->setDestIndex(destIndex);
//    enqMsg->setDestAddr(contactInfo->getAddress(destIndex));
    enqMsg->setOwnerAddr(getSelfIPAddress());
    enqMsg->setOwnerIndex(host->getIndex());
    enqMsg->setDestContactDuration(contactInfo->getContactDuration(destIndex));
    enquireSent++;
    emit(enquireRecSignal,enquireSent);
    sendEPFPacket(enqMsg,addressType->getBroadcastAddress(),1,0);
}

EPFRREQ *EPFRouting::createRREQ(const L3Address& destAddr)
{
    EPFRREQ *rreqPacket = new EPFRREQ("EPF-RREQ");
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        EPFRouteData *routeData = check_and_cast<EPFRouteData *>(lastKnownRoute->getProtocolData());
...
EPFRREP *EPFRouting::createRREP(EPFRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    EPFRREP *rrep = new EPFRREP("EPF-RREP");
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // node is itself the requested destination
        ...
        EPFRouteData *destRouteData = check_and_cast<EPFRouteData *>(destRoute->getProtocolData());
        EPFRouteData *originatorRouteData = check_and_cast<EPFRouteData *>(originatorRoute->getProtocolData());
...
EPFRREP *EPFRouting::createGratuitousRREP(EPFRREQ *rreq, IRoute *originatorRoute)
{
    ASSERT(originatorRoute != nullptr);
    EPFRREP *grrep = new EPFRREP("EPF-GRREP");
    EPFRouteData *routeData = check_and_cast<EPFRouteData *>(originatorRoute->getProtocolData());
...
CONTACTINFOMSG *EPFRouting::createCONTACTINFOMSG(){
    char msgName[20];
    sprintf(msgName,"CONTACTINFOMSG");
    CONTACTINFOMSG* contactMsg = new CONTACTINFOMSG(msgName);
    contactMsg->setHostIndex(host->getIndex());
    contactMsg->setSourceAddr(getSelfIPAddress());
    return contactMsg;
}

EnquireMessage *EPFRouting::createEnquireMessage(){
    EnquireMessage* enquire = new EnquireMessage("Enquire");
    enquire->setHostIndex(host->getIndex());
    enquire->setSourceAddr(getSelfIPAddress());
    return enquire;
}

DataMessage* EPFRouting::createDataMessage(){
    DataMessage* msg = new DataMessage("DataMessage");
    msg->setHostIndex(host->getIndex());
    msg->setSourceAddr(getSelfIPAddress());
    return msg;

}

void EPFRouting::handleRREP(EPFRREP *rrep, const L3Address& sourceAddr)
{
    EV_INFO << "EPF Route Reply arrived with source addr: " << sourceAddr << " originator addr: " << rrep->getOriginatorAddr()
    ...
    EPFRouteData *destRouteData = nullptr;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<EPFRouteData *>(destRoute->getProtocolData());
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (rrep->getAckRequiredFlag()) {
                    EPFRREPACK *rrepACK = createRREPACK();
                ...
                if (nextHopToDestRoute && nextHopToDestRoute->getSource() == this) {
                    EPFRouteData *nextHopToDestRouteData = check_and_cast<EPFRouteData *>(nextHopToDestRoute->getProtocolData());
                    nextHopToDestRouteData->addPrecursor(originatorRoute->getNextHopAsGeneric());
                }
                EPFRREP *outgoingRREP = rrep->dup();
...
void EPFRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    EPFRouteData *routingData = check_and_cast<EPFRouteData *>(route->getProtocolData());
...
void EPFRouting::sendEPFPacket(EPFControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
    ...
    udpPacket->setSourcePort(epfUDPPort);
    udpPacket->setDestinationPort(epfUDPPort);
...
void EPFRouting::handleRREQ(EPFRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{

    EV_INFO << "EPF Route Request arrived with source addr: " << sourceAddr << " originator addr: " << rreq->getOriginatorAddr()
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        EPFRouteData *routeData = check_and_cast<EPFRouteData *>(reverseRoute->getProtocolData());
    ...
    EPFRouteData *destRouteData = destRoute ? dynamic_cast<EPFRouteData *>(destRoute->getProtocolData()) : nullptr;
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // create RREP
        EPFRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
        ...
        destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag()) {
            ...
            EPFRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        EPFRREQ *outgoingRREQ = rreq->dup();
        forwardRREQ(outgoingRREQ, timeToLive);
    }
    else
        EV_WARN << "Can't forward the RREQ because of its small (<= 1) TTL: " << timeToLive << " or the EPF reboot has not completed yet" << endl;

    delete rreq;
}


IRoute *EPFRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    EPFRouteData *newProtocolData = new EPFRouteData();
    ...
    newRoute->setSourceType(IRoute::EPF);
...
void EPFRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj DETAILS_ARG)
{
...
void EPFRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    EPFRouteData *unreachableRouteData = check_and_cast<EPFRouteData *>(unreachableRoute->getProtocolData());
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getNextHopAsGeneric() == unreachableAddr) {
            EPFRouteData *routeData = check_and_cast<EPFRouteData *>(route->getProtocolData());
    ...
    EPFRERR *rerr = createRERR(unreachableNodes);
    rerrCount++;

    // broadcast
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendEPFPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());
}

EPFRERR *EPFRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    EPFRERR *rerr = new EPFRERR("EPF-RERR");
...
Coord EPFRouting::createReceiverCoord(double x,double y,long speed,simtime_t time, unsigned int type){
...
void EPFRouting::handleRERR(EPFRERR *rerr, const L3Address& sourceAddr)
{
    EV_INFO << "EPF Route Error arrived with source addr: " << sourceAddr << endl;
        ...
        if (route->getNextHopAsGeneric() == sourceAddr) {
            for (unsigned int j = 0; j < unreachableArraySize; j++) {
                if (route->getDestinationAsGeneric() == rerr->getUnreachableNodes(j).addr) {
                    ...
                    routeData->setIsActive(false);    // it means invalid, see 3. EPF Terminology p.3. in RFC 3561
                    routeData->setLifeTime(simTime() + deletePeriod);
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        EPFRERR *newRERR = createRERR(unreachableNeighbors);
        sendEPFPacket(newRERR, addressType->getBroadcastAddress(), 1, 0);
        rerrCount++;
    }
    delete rerr;
}

bool EPFRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void EPFRouting::handleWaitForRREP(WaitForRREP *rrepTimer)
{
    ...
    EPFRREQ *rreq = createRREQ(destAddr);
...
void EPFRouting::forwardRREP(EPFRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendEPFPacket(rrep, destAddr, 100, jitterPar->doubleValue());
}

void EPFRouting::forwardRREQ(EPFRREQ *rreq, unsigned int timeToLive)
{
    EV_INFO << "Forwarding the Route Request message with TTL= " << timeToLive << endl;
    sendEPFPacket(rreq, addressType->getBroadcastAddress(), timeToLive, jitterPar->doubleValue());
}

void EPFRouting::completeRouteDiscovery(const L3Address& target)
{
...
void EPFRouting::sendGRREP(EPFRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendEPFPacket(grrep, nextHop, timeToLive, 0);
}

EPFRREP *EPFRouting::createHelloMessage()
{
    ...
    EPFRREP *helloMessage = new EPFRREP("EPF-HelloMsg");
...
void EPFRouting::sendHelloMessagesIfNeeded()
{
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval)) {
        ...
        EPFRREP *helloMessage = createHelloMessage();
        sendEPFPacket(helloMessage, addressType->getBroadcastAddress(), 1, 0);
    }

    scheduleAt(simTime() + helloInterval - periodicJitter->doubleValue(), helloMsgTimer);
}

void EPFRouting::handleHelloMessage(EPFRREP *helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        EPFRouteData *routeData = check_and_cast<EPFRouteData *>(routeHelloOriginator->getProtocolData());
...
void EPFRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result EPFRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
    ...
    EPFRouteData *routeDestData = routeDest ? dynamic_cast<EPFRouteData *>(routeDest->getProtocolData()) : nullptr;
...
void EPFRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IRoute *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    EPFRouteData *unreachableRouteData = unreachableRoute ? dynamic_cast<EPFRouteData *>(unreachableRoute->getProtocolData()) : nullptr;
    ...
    rerrCount++;
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendEPFPacket(rerr, addressType->getBroadcastAddress(), 1, jitterPar->doubleValue());    // TODO: unicast if there exists a route to the source
}

void EPFRouting::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool EPFRouting::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        EPFRouteData *routeData = check_and_cast<EPFRouteData *>(route->getProtocolData());
...
EPFRREPACK *EPFRouting::createRREPACK()
{
    EPFRREPACK *rrepACK = new EPFRREPACK("EPF-RREPACK");
    rrepACK->setPacketType(RREPACK);
    rrepACK->setByteLength(2);
    return rrepACK;
}

void EPFRouting::sendRREPACK(EPFRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendEPFPacket(rrepACK, destAddr, 100, 0);
}

void EPFRouting::handleRREPACK(EPFRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void EPFRouting::handleCONTACTINFOMSG(CONTACTINFOMSG *contactInfoMsg){

    contactInfo->updateContactDuration(contactInfoMsg->getHostIndex(),simTime());
    contactInfo->setAddress(contactInfoMsg->getHostIndex(),contactInfoMsg->getSourceAddr());
    //contactInfo->setAddress(contactInfoMsg->getSourceAddr(),contactInfoMsg->getSourceAddr());


    EV << "Host index: " << contactInfoMsg->getHostIndex() << " Host address: " << contactInfoMsg->getSourceAddr() << endl;
    EV << "Mine index: " << host->getIndex() << " mine address: " << getSelfIPAddress() << endl;

    delete contactInfoMsg;
}

void EPFRouting::handleEnquireMessage(EnquireMessage* enqMsg){
    int threshold=2;
    L3Address destAddr = enqMsg->getDestAddr();
    L3Address selfAddr = getSelfIPAddress();


//    if(host->getIndex() != enqMsg->getHostIndex()){
    if(selfAddr != enqMsg->getOwnerAddr()){

        EV << "Enquire message from host: " << enqMsg->getSourceAddr() << endl;
        EV << "Message to: " << enqMsg->getDestAddr() << endl;
        EV << "My address: " << selfAddr << endl;

        EnquireResponse *resp = new EnquireResponse("Response");
        resp->setHostIndex(host->getIndex());
        resp->setSourceAddr(selfAddr);
        resp->setDestAddr(destAddr);
        resp->setRespondTo(enqMsg->getSourceAddr());
        resp->setDestIndex(enqMsg->getDestIndex());
        resp->setOwnerAddr(enqMsg->getOwnerAddr());
        resp->setOwnerIndex(enqMsg->getOwnerIndex());

//        if(host->getIndex()==enqMsg->getDestIndex()){
        if(selfAddr==destAddr){
            EV << "Enquire for me!" << endl;
            respOkSent++;
            destEnq++;
            destResp++;
            emit(respOkSentSignal,respOkSent);
            emit(destEnqSignal,destEnq);
            emit(destRespSignal,destResp);
            resp->setDestReached(true);
            resp->setOkToSend(true);
        }else{
            EV << "Enquire not for me!" << endl;
            //contact between communicating hosts
            if(contactInfo->getContactDuration(enqMsg->getHostIndex()).dbl() >= threshold){
                //contact between receiving host and destination host
                simtime_t duration = contactInfo->getContactDuration(enqMsg->getDestIndex());
                //contact between receiving host and destination host greater than contact between sender host and destination host
                if(duration.dbl() > enqMsg->getDestContactDuration().dbl()){
                    if(duration.dbl() >= threshold){
                        EV << "Ok to send!" << endl;
                        respOkSent++;
                        emit(respOkSentSignal,respOkSent);
                        ...
                        EV << "Not ok to send: source contact bigger than mine!" << endl;
                        respNOkSent++;
        ...
        sendEPFPacket(resp,addressType->getBroadcastAddress(),1,0);
    }
    delete enqMsg;
}

void EPFRouting::handleResponseMessage(EnquireResponse* enqMsg){

    if(enqMsg->getRespondTo() == getSelfIPAddress()){

        DataMessage* msg;
        if(enqMsg->getDestReached() || enqMsg->getOkToSend()){
            EV << "Destination "<< enqMsg->getDestAddr() << " reached" << endl;
            respOkRec++;
            emit(respOkRecSignal,respOkRec);
            //this node owns the data message
            if(enqMsg->getOwnerAddr()==getSelfIPAddress()){
                EV << "this node owns the data message" << endl;
                msg = createDataMessage();
                msg->setRespondTo(enqMsg->getSourceAddr());
                msg->setDestAddr(enqMsg->getDestAddr());
                msg->setDestIndex(enqMsg->getDestIndex());
                msg->setOwnerAddr(enqMsg->getOwnerAddr());
                msg->setOwnerIndex(enqMsg->getOwnerIndex());
                dataSent++;
                emit(dataSentSignal,dataSent);
                sendEPFPacket(msg,addressType->getBroadcastAddress(),1,0);
            }else{//another host owns the data message
                EV << "another host owns the data message" << endl;
                DataMessage* data = dataMsg[enqMsg->getOwnerAddr()];
                msg = createDataMessage();
                msg->setRespondTo(enqMsg->getSourceAddr());
                msg->setOwnerAddr(data->getOwnerAddr());
                msg->setDestAddr(data->getDestAddr());
                msg->setDestIndex(data->getDestIndex());
                msg->setOwnerAddr(data->getOwnerAddr());
                msg->setOwnerIndex(data->getOwnerIndex());
                EV << "Owner: " << enqMsg->getOwnerAddr() << endl;
                dataSent++;
                dataFwd++;
                emit(dataSentSignal,dataSent);
                emit(dataFwdSignal,dataFwd);
                sendEPFPacket(msg,addressType->getBroadcastAddress(),1,0);
                //delete dataMsg[enqMsg->getOwnerAddr()];
            }
        }/*else if(enqMsg->getOkToSend()){
            EV << "Destination not reached" << endl;
            respOkRec++;
            emit(respOkRecSignal,respOkRec);
            //this node owns the data message
            if(enqMsg->getOwnerAddr()==getSelfIPAddress()){
                ...
                EV << "another host owns the data message" << endl;
                msg = dataMsg[enqMsg->getOwnerAddr()];
                msg->setHostIndex(host->getIndex());
                msg->setSourceAddr(getSelfIPAddress());
                msg->setRespondTo(enqMsg->getSourceAddr());
                EV << "Owner: " << enqMsg->getOwnerAddr() << endl;
            ...
            respNOkRec++;
            emit(respNOkRecSignal,respNOkRec);
        }
    }
    delete enqMsg;
}

void EPFRouting::handleDataMessage(DataMessage* dataMsg){
    if(dataMsg->getDestAddr() == getSelfIPAddress()){
        dataDest++;
        emit(dataDestSignal,dataDest);
        EV << "Message: " << dataMsg->getDisplayString() << " from: " << dataMsg->getSourceAddr() << " to me" << endl;
        delete dataMsg;
    }else if (dataMsg->getRespondTo() == getSelfIPAddress()){//forward data message
        this->dataMsg[dataMsg->getOwnerAddr()] = dataMsg->dup();
        EnquireMessage* enqMsg = createEnquireMessage();
        enqMsg->setDestAddr(dataMsg->getDestAddr());
        enqMsg->setDestIndex(dataMsg->getDestIndex());
        enqMsg->setOwnerAddr(dataMsg->getOwnerAddr());
        enqMsg->setOwnerIndex(dataMsg->getOwnerIndex());
        enqMsg->setDestContactDuration(contactInfo->getContactDuration(dataMsg->getDestIndex()));
        enquireSent++;
        emit(enquireSentSignal,enquireSent);
        sendEPFPacket(enqMsg,addressType->getBroadcastAddress(),1,0);
    }
}

void EPFRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
/*void EPFRouting::initializeContactTable(){
    for(int i = 0; i < numHosts; i++)
        contactInfo.push_back(nullptr);
}*/
void EPFRouting::initializeContactInfo(){
        contactInfo = new EPFContactInfo();
}
/*void EPFRouting::updateContactInfo(EPFContactInfo *cInfo, L3Address& sourceAddr){
...
/*double EPFRouting::calcRouteWeight(const L3Address& nextHop){
    int i;
...
/*double EPFRouting::calcContactWeight(const L3Address& nextHop){
...
void EPFRouting::handleBlackListTimer()
{
