...
#include "inet/trickle/Trickletimer.h"
...
void IPv4::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        /*cModule* p_module = getContainingNode(this);
        cModule* module = p_module->getSubmodule("trickle");
        while (module == nullptr){
            p_module = p_module->getParentModule();
            module = p_module->getSubmodule("trickle");
        }*/
...
void IPv4::sendSelfMsg(cPacket *packet){
    IPv4Datagram* ipv4msg = check_and_cast<IPv4Datagram*>(packet);
    ipv4msg->setResended(false);
    if(trickle->getCounter() < trickle->par("MaxMessages").doubleValue()){
        send(ipv4msg,ipv4msg->getGateIndex());
    }else
        delete ipv4msg;
}
...
{
    EV<< "sendPacketToIeee802NIC" << endl;
...
void IPv4::sendPacketToNIC(cPacket *packet, const InterfaceEntry *ie)
{
    ...
    if(dynamic_cast<IPv4Datagram*> (packet) && strcmp(packet->getName(),"CONTACTINFO") && strcmp(packet->getName(),"DataMessage")){
       ...
       if(trickle->getExpectedId() == ipv4msg->getIdentification()){
           trickle->updateExpectedId();
           trickle->updateCounter();
       }else{
           if(trickle->getIntervalSize() > trickle->par("Imin").doubleValue()){
       ...
       if(trickle->getLastTime().dbl()==0){
         ...
         /*trickle->setLastTime(simTime());
          ...
          trickle->updateCurrInterval();
           ...
           if(ipv4msg->getResended()==false){
               ipv4msg->setResended(true);
               double delay = trickle->getCurrInterval()/100;
